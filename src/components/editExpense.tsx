...
const EditExpensePopup: React.FC<PopupProps> = ({onClose, getPeople, getExpenses, expenseId}) => {
    ...
    useEffect(() => {
        ...
        if (getExpenses.data && getPeople.data) {
            const expense = getExpenses.data.filter((expense: any) => expense.id === expenseId)[0];
            if (!expense) return;
            console.log(expense);
            setDescription(expense.description);
            setAmount(expense.amount);
            setPaidBy(JSON.stringify({personId: expense.paidBy, name: getPeople.data.filter((x: any) => x.personId === expense.paidBy)[0]?.name}));
            setSelected(expense.sharedWith.map((person: any) => ({ 
                personId: person.personId, 
                name: getPeople.data.filter((x: any) => x.personId === person.personId)[0]?.name, 
                amount: person.amount })));
            setCurrentPeople(getPeople.data.filter(
                (person: any) => !expense.sharedWith.some((shared: any) => shared.personId === person.personId)));
        }

    }, [getExpenses.data, getPeople.data, expenseId]);
    ...
    <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center z-50 text-black">
        <div className="flex flex-col justify-between bg-white p-8 rounded-lg shadow-lg space-y-4">
            ...
            <div className="flex mt-8 left-0 space-x-4 items-center">
                <div>Payer:</div>
                <select className="border border-2 w-48 py-2" name="Payer" id="payer" onChange={(e) => setPaidBy(e.target.value)} value={paidBy}>
